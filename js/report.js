...
// Request function
function request(searchdata, graph = false) {
    if ( typeof(searchdata) === 'undefined' )
        throw "Missing parameters";

    $('button.search-options, ul.tabs li.tab-link').addClass('disabled')

    $.get(searchdata.url, {searchdata: JSON.stringify(searchdata)}, function(data) {
       
        try {
            var report = new Report()
                report.render(data, function(data) {
                    $('button.search-options, ul.tabs li.tab-link').removeClass('disabled')
                })
        } catch (err) {
            console.log(err)
        }       
    ...
    if ( graph && searchdata.graph_url != false ) {
        try {
            dataGraph(searchdata.graph_url+'&period='+searchdata.datefrom+'-'+searchdata.dateto+'')
        } catch ( err ) { 
            console.log(err) 
        }
    }           
...
function varUpdate(searchdata) {
    ...
    if ( typeof(searchdata.request) !== 'undefined' && searchdata.request === 'daily' ) {
        var today = JSON.parse(document.getElementById('searchdata').value).date, contentTitle;

        contentTitle = (searchdata.date === today) ? 'FOR TODAY' : 'FOR '+searchdata.date;
            printTitle = 'FOR '+searchdata.date;
    } else {
        var today = JSON.parse(document.getElementById('searchdata').value).datefrom, contentTitle;
